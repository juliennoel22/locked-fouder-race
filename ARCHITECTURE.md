...
> Comprehensive system design, data flows, AI vision pipelines, and fault-tolerance patterns for Loreno.
...
flowchart TD
    ...
    subgraph API ["Next.js 15 App Router Backend"]
        ...
        AuthRoute["POST /api/auth/email (Passwordless)"]
    ...
    subgraph External ["External Services"]
        ...
        Stripe["Stripe Live Payment Link"]
    end
...
### 2.2. Multi-Tier AI Vision & Fallback Architecture
...
- **Resilience Strategy**: If the Gemini API returns a 429 rate limit or network timeout, [`app/api/scan/route.ts`](file:///home/julien/git/locked-fouder-race/app/api/scan/route.ts) seamlessly degrades to high-quality academic fallback data without crashing the UI.

### 2.3. Active Recall & Exam Gamification
...
| `POST` | `/api/auth/email` | Passwordless sign-in / registration via Supabase | No |
